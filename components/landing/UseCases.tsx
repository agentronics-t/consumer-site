...
export function UseCases() {
  return (
    <section id="use-cases" className="relative bg-bg-elevated/60 py-24 md:py-32">
      <div className="mx-auto max-w-6xl px-6">
        <div className="mb-14 max-w-2xl">
          ...
          <h2 className="font-display text-4xl font-bold leading-[1.05] tracking-tight text-text-primary md:text-5xl">
            ...
            <span className="font-sans italic text-border-glow">Agentronics can do it for you.</span>
          </h2>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4">
          {cases.map((c, i) => (
            <motion.div
              ...
              className="group rounded-xl border border-border bg-bg-card p-5 card-shadow transition-transform hover:-translate-y-0.5"
            >
              <div className="text-2xl">{c.emoji}</div>
              <div className="mt-3 font-display text-[15px] font-semibold text-text-primary">{c.title}</div>
              <div className="mt-1 text-[13px] leading-relaxed text-text-muted">{c.sub}</div>
            </motion.div>
          ))}
        </div>
      </div>
